...
//=> 메소드를 호출한 인스턴스의 정보를 저장하기 위한 참조 변수 
...
//=> 필드는 인스턴스마다 메모리(Heap)에 따로 생성되지만 인스턴스 갯수에 상관없이 메소드는 메모리에 하나만 생성된다 
//	메모리(Static)에 하나만 생성(Static 메모리 class 안) - 프로토타입 클래스(Prototype Class) 
//=> 메소드에서 필드 또는 메소드에 접근할 경우 명확하게 인스턴스를 구분하여 필드 또는 메소드에 접근하기 위해 this 필요  
//	메소드에 접근하기 위해 this 필요 

//this 매개변수를 사용하는 경우 
//1. 메소드에서 매개변수와 필드의 이름이 같은 경우 this를 이용하여 필드를 표현할 수 있다 
//=> this가 없으면 매개변수로 처리됨 
//2. 생성자에서 다른 생성자를 호출하기 위해 this를 사용  
//=> this로 생성자를 호출하는 명령 전에 다른 명령이 작성될 경우 에러 발생 
//3. 이벤트 처리 프로그램 또는 다중 스레드 프로그램의 메소드에서 인스턴스를 표현하여 처리 

//회원정보(아이디, 이름, 이메일)를 저장하기 위한 클래스 : VO(Value Object) 클래스 - VO클래스는 생성자 필요 
public class Member {
	//필드(Field) 선언 
	//=> 인스턴스 생성시 필드에 기본값 자동 저장(숫자형:0, 논리형:false, 참조형:null)
	//=> 인스턴스 생성시 필드에 저장된 기본값 변경 가능 
	private String id="NoId"; //= 붙여주는 것도 가능 
	private String name="NoName";
	private String email="NoEmail";
	
	//생성자(Constructor) 선언  
	// => 생성자를 하나도 선언하지 않으면 내부적으로 기본 생성자가 제공된다 
	...
	// => 초기화 작업 구현 : 생성 인스턴스의 필드에 원하는 초기값이 저장되도록 령을 작성하는 경우 
	
	//매개변수가 없는 생성자 - 기본 생성자와 동일한 형태
	// => 일반적으로 초기화 작업을 미구현 
	// => 선언하지 않을 경우 상속 관련 문제가 발생될 수 있으니 선언하는 것 권장 
	// 매개변수가 없는 생성자 자동 완성 : [Ctrl]+[Space] >> 목록 나오는데 Constructor 선택 
	public Member() { 
	...
	//매개변수가 선언된 생성자 자동 완성 : [alt]+[shift]+[S] >> 팝업메뉴 >> [O] >> 생성메뉴 >> 초기화필드 선택 >> Generate  
	...
	public void display() {
		...
		System.out.println("아이디 = "+this.id); //정석방법이지만  
		System.out.println("이름 = "+name); //but this. 쓰지 않아도 됨 
